...
def write_receipt(cc: Path, ctx_like: dict, events: list[dict], review_record: dict,
                  tests_fact: str, test_integrity: dict | None = None,
                  new_dependencies: list[str] | None = None) -> Path:
    ...
    new_dependencies is deps.new_dependency_lines()'s output over the same
    batch diffs (Task 3: supply-chain provenance) — unlike test_integrity,
    an empty/None list omits the section entirely rather than rendering an
    inert placeholder, so old receipts (and sessions that touched no
    dependency files) render byte-identically to before this feature.
    """
    ...
    if new_dependencies:
        lines.append("## Dependencies added this session")
        lines.extend(f"- {d}" for d in new_dependencies)
        lines.append("")
